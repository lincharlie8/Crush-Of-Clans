...
public class Player : MonoBehaviour {
	...
	*/
	...
	public Texture Bag,Fixed,BombPng,UpGrade,Black,Construction,tool1_0,tool1_1,tool2_0,tool2_1,wood,stone,metal,pulfur,cone,firePng,metalmine,pulfurmine,way;
	...
	public int x,y,z;//
	public bool Build=false,click=false,Bomb=false,BombGameStart=false,buttonEnable=false,bag=false,GetPosition=false,isHomeExsist=false,BombSelect=false,put=false,fontSizeedit;
	...
	public string infotext,GoldFinger;
	...
	//顯示資料
	public GameObject Trigger_ob;
	/// ///////////////////
	
	//網路用變數
	//Socket: 網路溝通, Thread: 網路監聽

	void setFontSize( int edit){
		
		guiSkin.box.fontSize += edit;
		guiSkin.label.fontSize += edit;
		guiSkin.button.fontSize += edit;
		guiSkin.textArea.fontSize += edit;
		guiSkin.textField.fontSize += edit;
		for (int i=0; i<8; i++) {
			guiSkin.customStyles[i].fontSize += edit;
	...
	void bigBomb(int status,string HouseID){
		...
		if(status==2){
			...
			infomationText("敵人在你的建築上設了地雷！你失去了身上的30％資源！！");
			for (int i =0; i<8; i++) {
		...
		if(status==3){
			...
			infomationText("敵人在你的建築上設了地雷！你失去了身上的60％資源！！");
			for (int i =0; i<8; i++) {
		...
		if(status==4){
			...
			infomationText("敵人在你的建築上設了地雷！你失去了身上的100％資源！！");
			for (int i =0; i<8; i++) {
	...
	void Update () {
		...
		/*if (click == true && State.joy!=null && Build != true && BombSelected!=true) {
			State.joy.GetComponent<joy_control>().enabled=false;
		} else {
			State.joy.GetComponent<joy_control>().enabled=true;
	...
	void OnGUI(){
		...
		/*if (GUI.Button (new Rect (0, 0, Screen.width / 10, Screen.height / 10), "SETTING", guiSkin.button)) {
			click=true;
			fontSizeedit=true;
		}*/
		...
		if(click==false ||BombSelected==true){
				...
				GUI.Box (new Rect (Screen.width*3/8, 0, Screen.width/4, Screen.height / 20),"",guiSkin.box);
				GUI.Box (new Rect (Screen.width*4/10, 0, Screen.width/10, Screen.height / 24),((int)(this.transform.position.x)).ToString(),guiSkin.textField);
				GUI.Box (new Rect (Screen.width*1/2, 0, Screen.width/10, Screen.height / 24),((int)(this.transform.position.z)).ToString(),guiSkin.textField);
				//座標/////End
			//插在OnGUI內，但這中間不要插入其他GUI繪製，不然會亂轉唷
			if(State.housePosX>=0 && State.housePosZ>=0){
				Matrix4x4 old_matrix = GUI.matrix;
				//取得角色位置
				Vector3 this_position = this.transform.position;
				//取得目的地位置，你在自己換成看到指向哪裡 << 這裡要改
				Vector3 destination = new Vector3(State.housePosX,0f,State.housePosZ);
				...
				//圖片跟skin你在換掉 << 這裡也要改
				GUI.Box (new Rect (100, 100, 100, 100), way , guiSkin.customStyles[3]);
...
/*if (GUILayout.Button ("Send", GUILayout.Height(50))) {
		...
		//GUI.Label(new Rect(screenPosition.x,screenPosition.y,100,100),"Player ID", style);
			if(info==false ){
					GUI.Label(new Rect(screenPosition.x-Screen.width/22,screenPosition.y, Screen.width/10, Screen.width/10),PlayerID,guiSkin.label);
				...
				if (Status!=0 && Trigger_ob!=null) {
					...
					if (Status == 1) {
						Source data = Trigger_ob.GetComponent<Source> ();
							if(data.kind==0){
								GUI.Label (new Rect (screenPosition.x, screenPosition.y, Screen.width/10, Screen.width/10), sourceName [data.kind] + "\n" + data.quatity[0],guiSkin.customStyles[5]);
							}else{
								GUI.Label (new Rect (screenPosition.x, screenPosition.y, Screen.width/10,  Screen.width/10), sourceName [1] + ":" + data.quatity[0]+ "\n" +sourceName [6] + ":" + data.quatity[1]+ "\n"+sourceName [7] + ":" + data.quatity[2] ,guiSkin.customStyles[5]);
								
							}
					} else if (Status == 2) {
						build data = Trigger_ob.GetComponent<build> ();
							GUI.Label (new Rect (screenPosition.x, screenPosition.y,  Screen.width/10, Screen.width/10), data.HouseName[data.kind] + " Lv" + data.HouseLevel + "\n" + data.PlayerID + "\nHp" + data.HP,guiSkin.customStyles[5]);
					} else if (Status == 3) {
						build data = Trigger_ob.GetComponent<build>();
							GUI.Label(new Rect(screenPosition.x,screenPosition.y, Screen.width/10, Screen.width/10),data.HouseName[data.kind] + " Lv" + data.HouseLevel + "\n" + data.PlayerID + "\nHp" + data.HP,guiSkin.customStyles[5]);
					} else if (Status == 4) {
		...
		if (bag == true) {
			...
			//GUI.Label (new Rect (Screen.width*3/8, Screen.height* 9/ 16 , Screen.width/3, Screen.height/8 ), "", guiSkin.label);
				...
				if(GUI.Button (new Rect (Screen.width*8/16, Screen.height* 7/ 13 , Screen.width/12, Screen.height/8 ),pulfurmine,guiSkin.customStyles[6])){
					...
					GoldFinger+="7";
					if (GoldFinger == "107") {
						int[] quantity=new int[8]{100,100,50,10,10,10,20,20};
					...
					if (GoldFinger == "507") {
						...
						float posx=r.Next(2000);
						float posz=r.Next(2000);
						Vector3 vec=new Vector3(posx,this.transform.position.y,posz);
						this.transform.position=vec;
						GoldFinger="";
					}
					if (GoldFinger == "607") {
						int[] quantity=new int[8]{1000,1000,1000,1000,1000,1000,200,200};
						sendSourceModify(quantity);
						GoldFinger="";
					}
				}
		...
		if(BombSelected==true){ 
				...
				if(bombKind<3){
					GUI.Box (new Rect (0, Screen.height / 5, Screen.width, Screen.height / 10),"選擇目標，直接對建築造成傷害。",guiSkin.box);
				}else{
					GUI.Box (new Rect (0, Screen.height / 5, Screen.width, Screen.height / 10),"選擇目標，陷害屋主失去資源。",guiSkin.box);
			...
			if (GUI.Button (new Rect (Screen.width* 5 / 6, Screen.height* 3 / 4 , Screen.width/6, Screen.height/4 ),BombPng,guiSkin.customStyles[3])){
					...
					if(triggerHouesNow.status!=5){
						...
						m_sound.PlayOneShot(isBomb);
						infomationText("這是敵人設計的陷阱！你失去了"+persent.ToString()+"個炸彈！！");
						Server.Send ("28"+((bombKind%3) +1).ToString()+","+(persent*-1).ToString());
	...
	/*	if (5 - (int)(Time.time - BombTimeCount) >= 1) {
			...
			case 0:
				...
				for(int i=0;i<4;i++){
					...
					if (GUI.Button (new Rect (Screen.width*w/4, Screen.height* h/ 3 , Screen.width/4, Screen.height/3 ),BombGame[i].ToString(),guiSkin.button)){
							m_sound.PlayOneShot(buttonSound);
			...
			if (Build==false && GUI.Button (new Rect (Screen.width* 5 / 6, Screen.height* 3 / 4 , Screen.width/6, Screen.height/4 ),FunctionButton[Status],guiSkin.customStyles[3])){
								...
								if(kind==0){
									if(quatity[0]<=pick[tool,toolKind]){
										...
										//	pickup.quatity[0]-=getQutity[0];
											...
											getQutity[0]=5;;//pick[tool,toolKind];
									...
									getQutity[0]=(pickup.quatity[0]>=5)?5:pickup.quatity[0];
									...
									getQutity[2]=(pickup.quatity[1]>=1)?1:0;
								
									if(pickup.quatity[0]-getQutity[0]<=0 &&pickup.quatity[1]-getQutity[1]<=0&&pickup.quatity[2]-getQutity[2]<=0){
										State.SourceUpdate[pickup.SourceID]=0;
									}
				...
				case 2:
					...
					build StockNow = TriggerHouse.GetComponent<build>();
						if(StockNow.status==2 || StockNow.status==3 ||StockNow.status==4){
				...
				case 3:
					build WorkNow = TriggerHouse.GetComponent<build>();
						if(WorkNow.status==2 || WorkNow.status==3 ||WorkNow.status==4){
				...
				case 4:
					build ScienceNow = TriggerHouse.GetComponent<build>();
						if(ScienceNow.status==2 || ScienceNow.status==3 ||ScienceNow.status==4){
				...
				case 6:
					build HouseNow = TriggerHouse.GetComponent<build>();
						if(HouseNow.status==2 || HouseNow.status==3 ||HouseNow.status==4){
					...
					break;
					case 7:
						build StruNow = TriggerHouse.GetComponent<build>();
						if(StruNow.status==2 || StruNow.status==3 ||StruNow.status==4){
							bigBomb(StruNow.status,StruNow.HouseID);
							StruNow.status=1;
					...
					case 8:
						...
						if(B101Now.status==2 || B101Now.status==3 ||B101Now.status==4){
		...
		if (fontSizeedit == true) {
				// 蓋房子放這邊
				GUI.Box (new Rect (Screen.width / 2 - (Screen.width / 4), Screen.height / 2 - (Screen.height / 4), Screen.width / 2, Screen.height / 2), "", guiSkin.box);
				GUI.Label (new Rect (Screen.width*1/4, Screen.height*1/2-Screen.height*1/10, Screen.width*1/2, Screen.height*1/10), "是否要離開？", guiSkin.customStyles[1]);
